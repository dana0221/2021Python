...
class Adoption_book:
    def __init__(self):
        ...
        # 처음 객체 생성 시 무조건 로그인
        if self.set_user() == False:
            # 회원가입이랑 로그인 둘다 안되었을 때
            print('로그인 실패 ! ')

    # 새로 로그인
    def set_user(self):
        if input('회원가입 : 0 | 로그인 : 1 >> ') == 1:
            sainup_id = input('아이디 입력 : ')
            sainup_pw = input('비밀번호 입력 : ')

            # 로그인 한 이름과 비밀번호가 일치하면 로그인 성공
            for i in self.users:
                if sainup_pw == i.pw and sainup_id == i.id:
                    self.now_user = i
                    print('로그인 성공')
                    return True
                    break
    ...
    # 게시물 등록
    def up_animal(self):
        # 현재 사용자 객체의 올린 게시물 리스트에 게시물 올린거 추가
        self.user.up_list.append(Parcel_out())
